...
class CountyPromoForeignImport implements ToModel
{
    ...
    */
    ...
    {
        if($exist_promo_code = CountyPromoRelation::where('county_promo_id',$this->promo_code_id)->where('county',$row[0])->where('price',$row[1])->where('county_type',1)->first()){
